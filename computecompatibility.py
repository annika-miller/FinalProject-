...
def main():
    ...
    for row in result:
        ...
        self = [row[i] for i in range(1,int((len(row)-1)/2 + 1))] #self-responses go in one array
        other = [row[i] for i in range(int((len(row)-1)/2 + 1), len(row))] #others responses go in other array
